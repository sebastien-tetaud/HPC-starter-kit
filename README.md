...
Common issues with multi-GPU training:
...
3. **Adjust network interface:**
Edit in PBS script or [examples/ddp_cnn.py](examples/ddp_cnn.py#L84-L87):
```bash
export NCCL_SOCKET_IFNAME=ib0  # Or eth0, ib1, etc.
```
...
- Use appropriate walltime (avoid over-requesting)
- Test with smaller jobs first

### 2. Data Management
- Use fast local storage (e.g., `/scratch`, `/tmp`) for I/O
- Copy data to local node storage before training
- Clean up temporary files after jobs complete

### 3. Distributed Training
- Match `--nproc_per_node` with `ngpus` in PBS script
- Use `DistributedSampler` for data loading
- Implement gradient synchronization correctly
- Monitor GPU utilization with `nvidia-smi`

### 4. Debugging
- Test scripts locally before submitting jobs
- Start with small batch sizes and datasets
- Enable verbose logging for initial runs
...
For training across multiple nodes, modify PBS script:

```bash
#PBS -l select=2:ncpus=96:mem=700g:ngpus=4  # 2 nodes, 8 GPUs total
...
torchrun \
  ...
  examples/ddp_cnn_wandb.py
```

### Custom Network Configuration

For specific network topologies, customize NCCL settings:

```bash
export NCCL_IB_DISABLE=0        # Enable InfiniBand
export NCCL_NET_GDR_LEVEL=5     # GPUDirect RDMA level
export NCCL_P2P_LEVEL=NVL       # P2P communication level
```
...
This starter kit is based on best practices from HPC PyTorch deployments and includes configuration patterns for common cluster environments.
